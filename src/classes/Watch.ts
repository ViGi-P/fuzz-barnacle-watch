...
import { WatchResponse, SubscriptionResponse } from "../interfaces";
...
export class Watch {
	...
	constructor(
		...
		this.directory =
			directory === "." ? process.cwd() : `${process.cwd()}/${directory}`;
	...
	private addSubscription() {
		...
		this.client.command(
			["subscribe", this.directory, this.directory, sub],
			(error) => {
				if (error) {
					this.removeWatch();
					throw error;
	...
	private spawnSynchronously() {
		const execute = (command: string, i: number) => {
			...
			child.on("exit", (status, signal) => {
				...
				if (
					this.commands[i + 1] &&
					exitSignals.every((sig) => sig !== signal)
				) {
	...
	private removeWatch() {
		this.client.command(["watch-del", this.directory], (error, resp) => {
			if (error) {
